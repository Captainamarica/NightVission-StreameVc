...
async def ping_pong(_, m: Message):
    ...
    await m_reply.edit_text(
        f"{emoji.ROBOT} **ᴘᴏɴɢ** : `{delta_ping * 1000:.3f} ms`"
    )
...
async def get_uptime(_, m: Message):
    ...
    await m.reply_text(
        f"{emoji.ROBOT} **ᴅᴇᴠᴜ ʀᴀᴅɪᴏ ᴘʟᴀʏᴇʀ🧚‍♀️**\n"
        f"- **ᴜᴘᴛɪᴍᴇ:** `{uptime}`\n"
        f"- **ʀᴇsᴛᴀʀᴛᴇᴅ ᴛɪᴍᴇ:** `{START_TIME_ISO}`"
    )


@Client.on_message(
    filters.command("sinfo", prefixes="?")
    & (filters.group | filters.private)
    & self_or_contact_filter
    & ~filters.edited
    & ~filters.bot
...
async def get_sysinfo(client, m: Message):
    response = "**sʏsᴛᴇᴍ ɪɴғᴏs**:\n"
    m_reply = await m.reply_text(f"💥")
